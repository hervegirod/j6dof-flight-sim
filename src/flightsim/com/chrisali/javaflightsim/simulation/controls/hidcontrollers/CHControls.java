            ...
            if (componentIdentifier.getName().matches("^[0-9]*$")) { // If the component identifier contains only numbers, it is a button
               if (component.getPollData() == 1.0f && controller.getName().toLowerCase().compareTo("ch flight sim yoke usb") == 0) {
                  ...
                  switch (component.getIdentifier().toString()) {
                     case "2":
                        if (trimAileron >= FlightControlType.AILERON.getMinimum()) {
                           trimAileron += getDeflectionRate(FlightControlType.ELEVATOR) / 10;
                        }
                        break;
                     case "3":
                        if (trimAileron <= FlightControlType.AILERON.getMaximum()) {
                           trimAileron -= getDeflectionRate(FlightControlType.ELEVATOR) / 10;
                        }
                        break;
                     case "4":
                     ...
                     case "5":
                        ...
                        break;
                     case "6":
                        if (flaps >= FlightControlType.FLAPS.getMinimum()) {
                           controls.put(FlightControlType.FLAPS, (flaps -= getDeflectionRate(FlightControlType.FLAPS)));
                        }
                        break;
                     case "7":
                        if (flaps <= FlightControlType.FLAPS.getMaximum()) {
                           controls.put(FlightControlType.FLAPS, (flaps += getDeflectionRate(FlightControlType.FLAPS)));
                        }
                        break;
                     case "10":
                        if (trimElevator <= FlightControlType.ELEVATOR.getMaximum()) {
                           trimElevator += getDeflectionRate(FlightControlType.AILERON) / 20;
                        }
                        break;
                     case "11":
                        if (trimElevator >= FlightControlType.ELEVATOR.getMinimum()) {
                           trimElevator -= getDeflectionRate(FlightControlType.AILERON) / 20;
                        }
                        break;
                  }
